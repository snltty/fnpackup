...
### [💼]打包fpk
1. 创建应用，在应用列表目录
    1. 创建应用，选择原生或Docker，有无UI入口
    2. 导入已有的.fpk文件
2. 双击应用文件夹进入应用
3. 上传程序
    1. **原生应用：** 可以上传你的程序到`app/server`中
    2. **Docker应用：** 直接编辑`app/docker/docker-compose.yaml`文件即可
4. 编辑方式
    1. 可以使用`快速编辑`，包含了所需编辑的内容，一一编辑即可
    2. 或双击文件单独编辑
    3. 或右键文件源码编辑
5. 编辑好全部内容后打包fpk，在项应用文件夹下查看生成的fpk文件

### [🌍]静态托管

如果你有一些静态网页，或纯静态内容的fpk，专门写一个HTTP/cgi服务,都实在太麻烦了，那么可以使用本程序进行托管
