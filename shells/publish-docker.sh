...
do
	...
	do
		...
		docker buildx build -f ${target}/public/publish/docker/linux-${p}-x64/${f}/Dockerfile --platform="linux/x86_64"  --force-rm -t "${image}-${p}-x64:latest" -t "${image}-${p}-x64:v0.0.1" . --push
		cd ../../../../../

		cd public/publish/docker/linux-${p}-arm64/${f}
		docker buildx build -f ${target}/public/publish/docker/linux-${p}-arm64/${f}/Dockerfile --platform="linux/arm64"  --force-rm -t "${image}-${p}-arm64:latest" -t "${image}-${p}-arm64:v0.0.1" . --push
		cd ../../../../../

        cd public/publish/docker/linux-${p}-arm/${f}
		docker buildx build -f ${target}/public/publish/docker/linux-${p}-arm/${f}/Dockerfile --platform="linux/arm/v7"  --force-rm -t "${image}-${p}-arm:latest" -t "${image}-${p}-arm:v0.0.1" . --push
		cd ../../../../../
	done
done